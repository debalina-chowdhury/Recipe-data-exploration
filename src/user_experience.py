...
def high_rating_recipes_wordcloud(Merged_data):
    ...
    wordcloud = WordCloud(width = 800, height = 800,
                    ...
                    min_font_size = 10).generate(high_rating_recipes_reviews)
...
def yearwise_highest_zero_rating_recipes(Merged_data):
    '''This function develops a wordcloud of the recipes with highest number of top ratings for each year
    param Merged_data: a pandas DataFrame
    return: None
    '''
    assert isinstance(Merged_data,pd.DataFrame), "Please provide valid Dataframe"
    low_rating_recipes=Merged_data[Merged_data['rating']==0.0].groupby(['year','id','name','ingredients'])['rating'].count().reset_index(name='highest_zero_rating_count')
    low_rating_recipes=low_rating_recipes.sort_values(by=['year','highest_zero_rating_count'], ascending=False)
    low_rating_recipes=low_rating_recipes[['year','name','ingredients','highest_zero_rating_count']].groupby(['year']).head(1).reset_index()
    low_rating_recipes.pop('index')
    low_rating_recipes.pop('ingredients')
    plt.figure(figsize=(10, 5))
    plt.rc('font', size=13) 
    plt.barh(low_rating_recipes['name']+'('+low_rating_recipes['year'].astype('str')+')', low_rating_recipes['highest_zero_rating_count'],align='center')
    plt.xticks(rotation=90)
    plt.title('Yearwise highest zero rating recipes')
    plt.xlabel('recipes')
    plt.ylabel('zero rating frequency')
    plt.show()